...
namespace Cloud.Communication.Packets.Outgoing.Moderation
{
    class ModeratorUserInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserInfoMessageComposer)
        {
			...
			WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((CloudServer.GetUnixTimestamp() - Convert.ToDouble(User["account_created"])) / 60)) : 0);
			WriteInteger(User != null ? Convert.ToInt32(Math.Ceiling((CloudServer.GetUnixTimestamp() - Convert.ToDouble(User["last_online"])) / 60)) : 0);
			WriteBoolean(User != null ? CloudServer.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(User["id"])) != null : false);
