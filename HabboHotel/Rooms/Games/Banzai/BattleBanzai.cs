...
using Cloud.Utilities.Enclosure;
using System.Linq;
...
namespace Cloud.HabboHotel.Rooms.Games.Banzai
{
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        if (CloudServer.GetUnixTimestamp() - timestarted > 5)
                        {
                            CloudServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            CloudServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            if (CloudServer.GetUnixTimestamp() - timestarted > 5)
                                CloudServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);
