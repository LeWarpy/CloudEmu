    ...
    public class RoomItemHandling
    {
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = CloudServer.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (newItem)
            {
                ...
                {
                    if (Session != null)
                        Session.SendNotification(CloudServer.GetGame().GetLanguageManager().TryGetValue("room.item.already_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }
